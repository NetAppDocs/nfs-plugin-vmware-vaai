...
* If you are using NetApp ONTAP tools for VMware vSphere to install the plug-in, start with <<step3,Step 3>>.
...
+
link:task_configuring_export_policies_for_clustered_data_ontap_to_allow_vaai_over_nfs.html[Configuring export policies for ONTAP to allow VAAI over NFS]
+
...
+
For more information about the `exportfs` command, see the https://library.netapp.com/ecm/ecm_download_file/ECMP1401220[Data ONTAP File Access and Protocols Management Guide for 7-Mode].
+
